        ...
        unsafe {
            loop {
                match &*inner.add(*current_position) {
                    ...
                    Some(value) => {
                        let key = K::from_index_unchecked(*current_position);
                        *current_position += 1;
                        *len -= 1;
        ...
        unsafe {
            loop {
                match &mut *self.inner.add(self.current_position) {
                    ...
                    Some(value) => {
                        let key = K::from_index_unchecked(self.current_position);
                        self.current_position += 1;
                        self.len -= 1;
                        return Some((key, value));
                    }
    ...
    fn next(&mut self) -> Option<(K, V)> {
        ...
        while let Some(back) = inner.pop() {
            if let Some(value) = back {
                let key = unsafe { K::from_index_unchecked(inner.len()) };
                *len -= 1;
                return Some((key, value));
            }
        }
        ...
        K: BlazeMapId
{
    ...
    fn next(&mut self) -> Option<(K, V)> {
        ...
        unsafe {
            loop {
                let value = &mut *iter.inner.add(iter.current_position);
                match value.take() {
                    None => {
                        iter.current_position += 1;
                        continue;
                    }
                    Some(value) => {
                        let key = K::from_index_unchecked(iter.current_position);
                        iter.current_position += 1;
                        iter.len -= 1;
                        return Some((key, value));
                    }
...
impl<'a, K, V> Copy for Keys<'a, K, V> {}
...
impl<'a, K, V> Copy for Values<'a, K, V> {}
